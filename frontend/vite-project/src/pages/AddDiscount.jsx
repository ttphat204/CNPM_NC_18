...
function AddDiscount() {
    ...
    const validateForm = () => {
        ...
        if (discount === '' || discount < 0) errors.discount = 'Phần trăm giảm giá phải là số dương.';
        if (!category) errors.category = 'Cần chọn danh mục.';
        if (!startDate) errors.startDate = 'Ngày bắt đầu là bắt buộc.';
        if (startDate && startDate < today) errors.startDate = 'Ngày bắt đầu phải là ngày hiện tại hoặc ngày sau đó.';
        if (!endDate) errors.endDate = 'Ngày kết thúc là bắt buộc.';
        if (endDate && endDate <= today) errors.endDate = 'Ngày kết thúc phải lớn hơn ngày hiện tại.';
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('http://localhost:5000/api/discounts', { // Điều chỉnh endpoint API theo nhu cầu
                method: 'POST',
            ...
            <main className="flex-1 p-6 bg-gray-100">
                <div className="max-w-lg mx-auto bg-white p-6 rounded-md shadow-md">
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className="mb-4">
                            <label className="block text-gray-700 font-bold mb-2">Tên Mã Giảm Giá</label>
                            <input
                                type="text"
                                value={name}
                                onChange={(e) => setName(e.target.value)}
                                className={`w-full px-3 py-2 border rounded-md ${errorMessages.name ? 'border-red-500' : ''}`}
                        ...
                        <div className="mb-4">
                            <label className="block text-gray-700 font-bold mb-2">Giảm giá (%)</label>
                            <input
                                type="text" // Change type to text
                                value={discount}
                                onChange={(e) => setDiscount(e.target.value)}
                                className={`w-full px-3 py-2 border rounded-md ${errorMessages.discount ? 'border-red-500' : ''}`}
                                placeholder="Nhập phần trăm giảm giá"
                                inputMode="numeric" // Optional: To display numeric keyboard on mobile devices
                            />
                            {errorMessages.discount && <p className="text-red-500 text-sm">{errorMessages.discount}</p>}
                        </div>
                        ...
                        <div className="mb-4">
                            ...
                            <select
                                ...
                                className={`w-full px-3 py-2 border rounded-md ${errorMessages.category ? 'border-red-500' : ''}`}
                            >
                        ...
                        <div className="mb-4">
                            ...
                            <input
                                ...
                                className={`w-full px-3 py-2 border rounded-md ${errorMessages.startDate ? 'border-red-500' : ''}`}
                            />
                            {errorMessages.startDate && <p className="text-red-500 text-sm">{errorMessages.startDate}</p>}
                        </div>
                        ...
                        <div className="mb-4">
                            ...
                            <input
                                ...
                                className={`w-full px-3 py-2 border rounded-md ${errorMessages.endDate ? 'border-red-500' : ''}`}
                            />
                            {errorMessages.endDate && <p className="text-red-500 text-sm">{errorMessages.endDate}</p>}
                    ...
                    {successMessage && <p className="mt-4 text-green-500 font-bold">{successMessage}</p>}
                </div>
            </main>
        </div>
    );
